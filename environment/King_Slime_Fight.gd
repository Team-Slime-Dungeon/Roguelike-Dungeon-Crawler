...
func _ready():
	...
	if fight_state == 0 and end_fight == false:
		...
		for i in range(12,15):
			for j in range(20,23):
				enemy_spawn_pads.append(Vector2(i,j))
		# Top left side
		...
		for i in range(-4,10):
			support_spawn_pads.append(Vector2(i,-4))
			
		for i in range(14,19):
			support_spawn_pads.append(Vector2(i,-2))
		
		for i in range(12,16):
			support_spawn_pads.append(Vector2(9,i))
		
		for i in range(4,8):
			support_spawn_pads.append(Vector2(-10,i))
		
		for i in range(17,21):
			support_spawn_pads.append(Vector2(-10,i))
		
		place_supports(15, [0, 30])
		place_supports(5, [0, 0, 30, 51])
	...
	if fight_state == 100 and end_fight == true:
		...
		Items.Player_Inventory._add_item(72, 1) 
		fight_state = 101
...
func spawn_slimes(spawn_num):
	for i in range(spawn_num):
		...
		var spawn_spot = enemy_spawn_pads[0]
...
func place_supports(vase_num, loot_table = [0,30]):
	for i in range(vase_num):
		...
		support_spawn_pads.shuffle()
		var support_spawn_spot = support_spawn_pads[0] #[randi() % support_spawn_pads.size()]
		support_spawn_pads.erase(support_spawn_spot)

		support_spawns.append(spawnedItem)
		add_child(spawnedItem)
			
		support_spawns[support_totals].global_position = $BossDeco.map_to_local(support_spawn_spot) /2
